...
public class RZJLActivity extends BaseActivity implements View.OnClickListener {
    ...
    private RZJLHistoryAdapter historyAdapter;
    ...
    public void initViews() {
        ...
        if (bkKsList.size() != 0) {
            ...
            List<RzjlHistoryViw> historyViwList = new ArrayList<>();
            for (String a : remap.keySet()) {
                List<String> b = remap.get(a);
                for (String c : b) {
                    RzjlHistoryViw historyViw = new RzjlHistoryViw();
                    ...
                    historyViw.setWrz_sl(DbServices.getInstance(getBaseContext()).queryBkKsWTG(a, c, "1"));
                    historyViw.setAll_sb(DbServices.getInstance(getBaseContext()).selectKCCCrzjg(a, c).size());
                    historyViw.setYSb_sl(DbServices.getInstance(getBaseContext()).selectWSBrzjg(a, c, "1").size());
