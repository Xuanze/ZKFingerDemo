...
import com.zhongruan.android.zkfingerdemo.adapter.DJJLHistoryAdapter;
...
import com.zhongruan.android.zkfingerdemo.ui.DJXQActivity;
...
public class DJJLFragment extends RxFragment implements View.OnClickListener {
    ...
    private DJJLHistoryAdapter djjlHistoryAdapter;
    private List<List<DjjlHistoryViw>> historyLists;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_djjl, container, false);
        initView(view);
        return view;
    }
    ...
    private void initView(View view) {
        ...
        mLlXqView = view.findViewById(R.id.ll_xq_view);
        ...
        mListItemPiechartYyz.setText(DbServices.getInstance(getActivity()).queryBkKsIsTG(kcmc, ccmc, "2") + "");
        mListItemPiechartWyz.setText(bk_ks.size() - DbServices.getInstance(getActivity()).queryBkKsIsTG(kcmc, ccmc, "2") + "");

        rzjg1 = DbServices.getInstance(getActivity()).selectWSBrzjg2(kcmc, ccmc, "23", "1");
        rzjg2 = DbServices.getInstance(getActivity()).selectKCCCrzjg2(kcmc, ccmc, "23");
        ...
        mListItemPiechart.setData(setPiechartData(DbServices.getInstance(getActivity()).queryBkKsIsTG(kcmc, ccmc, "2"), bk_ks.size()));
        ...
        if (bkKsLs.size() != 0) {
            ...
            List<DjjlHistoryViw> rzjlHistoryViwList = new ArrayList<>();
            for (String a : remap.keySet()) {
                List<String> b = remap.get(a);
                for (String c : b) {
                    DjjlHistoryViw djjlHistoryViw = new DjjlHistoryViw();
                    djjlHistoryViw.setCc_mc(c);
                    djjlHistoryViw.setKc_mc(a);
                    LogUtil.i("lhj", a + "  -------------  " + c);
                    djjlHistoryViw.setAll_sl(DbServices.getInstance(getActivity()).queryBKKSList(a, c).size());
                    djjlHistoryViw.setDj_sl(DbServices.getInstance(getActivity()).queryBkKsIsTG(a, c, "1"));
                    djjlHistoryViw.setZc_sl(DbServices.getInstance(getActivity()).queryBkKsWTG(a, c, "1"));
                    djjlHistoryViw.setAll_sb(DbServices.getInstance(getActivity()).selectKCCCrzjg(a, c).size());
                    djjlHistoryViw.setYSb_sl(DbServices.getInstance(getActivity()).selectWSBrzjg(a, c, "1").size());
                    rzjlHistoryViwList.add(djjlHistoryViw);
                }
            }
            ...
            mEplvStatistic.setOnGroupExpandListener(new ExpandableListView.OnGroupExpandListener() {
                ...
                public void onGroupExpand(int groupPosition) {
                    ...
                    setListHeight(mEplvStatistic, djjlHistoryAdapter);
                }
            });
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.list_item_statistic_history_item:
                Intent intent = new Intent(getActivity(), DJXQActivity.class);
                intent.putExtra("djjlkc", kcmc);
                intent.putExtra("djjlcc", ccmc);
     ...
     */
    public void setListHeight(ExpandableListView listView, DJJLHistoryAdapter listAdapter) {
        if (listAdapter == null) {
            return;
        }
        int totalHeight = 0;
    ...
    private PieData setPiechartData(int paramInt1, int paramInt2) {
        ...
        if (paramInt2 != 0) {
            ...
            pieEntries.add(new PieEntry(arrayOfFloat[1], "正常"));
        }
